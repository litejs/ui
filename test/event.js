...
describe("event", function() {
	...
	lib.El.asEmitter(Emitter.prototype)
	...
	it ("should not polute global object", function(assert, mock) {
		var em = new Emitter()
		, fn = mock.fn()
		em.on.call(lib, "x", fn)
		assert(lib._e === undefined)
		assert.equal(em.emit.call(lib, "x"), 1)
		em.off.call(lib, "x", fn)
		assert.equal(em.emit.call(lib, "x"), 0)
		assert.equal(fn.called, 1)
		assert.end()
	})

	it ("should handle events", function(assert) {
		assert
